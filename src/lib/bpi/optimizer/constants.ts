...
export const BpiOptimizerConstants = {
  ...
  /**
   * 候補選択の多様性ボーナス（純粋な効率ランキングへの製品判断による補正）。
   *
   * シフト法べき乗平均（`docs/bpi-math.md` §4.3）は総合BPIより既に高い曲ほど
   * `∂T/∂BPI_i`が大きい「上位曲支配」の性質を持つため、`trueBpiGain/exGain`を
   * そのまま最大化すると「既に得意な曲をわずかに伸ばす」提案に収束しがちで、
   * 実際のプレイ体験としては単調・非現実的になる。効率ランキング自体（＝候補の
   * 選ばれやすさ）にのみ倍率をかけて多様性を持たせる。報告する`bpiGain`・
   * `efficiency`の値自体はこの補正の影響を受けない（あくまで選び方の重み付け）。
   */
