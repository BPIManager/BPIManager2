 ...
 *
 * アンカーの決め方は実データでの検証を踏まえたハイブリッド:
 * - z100（BPI100）は「歴代全一=100」という原典の定義を崩さないよう、
 *   全曲共通の定数ではなく曲ごとに実際の全一のz値を使う
 * - z0（BPI0）は全曲共通の定数（`NEW_BPI_Z0`）を使う。皆伝合格者ではなく
 *   アリーナA帯在籍者(a1〜a5)の潜在能力a_iの中央値で、実測 ≈ -0.193。
 *   「BPI0＝皆伝平均」という当初の前提は撤回し、実在する競技人口の
 *   典型的な実力を直接の基準に置き換えている
 *
