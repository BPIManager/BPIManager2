...
function useCountUp(target: number | null, active: boolean, delay = 0) {
...
export const ArenaSection = ({ arena }: Props) => {
  ...
  const a1Ref = useCountUp(arena.maxA1Continue, inView, 0.7);
  ...
  return (
    <>
      ...
      >
        ...
        <p
          className="mt-12 max-w-sm text-center text-sm leading-relaxed"
          style={{
        ...
        >
          {[
            `今月のアリーナ最高クラスは ${arena.bestClass} でした。`,
            arena.bestRank != null ? `最高順位は ${arena.bestRank} 位。` : null,
            arena.maxA1Continue != null && arena.maxA1Continue > 0
              ? `A1を${arena.maxA1Continue}回維持しました。`
              : null,
          ]
            .filter(Boolean)
            .join(" ")}
