...
export const GlobalRankingContainer = () => {
  ...
  const isRadarCategory = category !== "totalBpi";
  const isLatestVersion = version === latestVersion;

  const { data, isLoading } = useGlobalRanking(version, category);
  ...
  const handleVersionChange = (v: string) => {
    router.push(
      { query: { ...router.query, version: v, category: "totalBpi" } },
      undefined,
      { shallow: true },
    );
  };
  ...
  const { totalCount, selfRank, viewerRadar } = data;
    ...
    <>
      <div className="flex gap-3 mb-4">
        ...
        <div className="flex flex-col gap-1 flex-1">
          ...
          <Select
            value={isLatestVersion ? category : "totalBpi"}
